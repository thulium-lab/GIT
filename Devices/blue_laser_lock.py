...
""" РАБОЧАЯ ВЕРСИЯ ПРОГРАММЫ ПЕЕРЕХАЛА В ДРОПБОКС К ОСТАЛЬНЫМ ПРОГРАММАМ ПО УПРАВЛЕНИЮ ПРИБОРАМИ
...
def start_correction(srs,sacher,threshold_error=0.003,correction_limit=0.05):
    ...
    while(True):
        ...
        if is_lock_on:
            ...
            print('SRS output = %.3f mV ; Sacher offset = %.3f V ; overload = %i'%(1e3*error,get_sacher_offset(sacher),ovld))
        if not is_lock_on:
            print('Lock is off')
...
if __name__ == '__main__':
    ...
    #     srs.write(b'*IDN?\r')
        print('Exit...')
        exit(1)
    sacher = connect_port(names=['Sacher Lasertechnik', ' PilotPC 500',' SN14098015'],default_port='COM6',baudrate=57600)
    if sacher == None:
